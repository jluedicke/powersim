...
program define pll_powersim, rclass
...
*----------------------------------------------
	...
	if "`dryrun'" != "" {
		preserve
			...
			if "`fam'"=="binomial" {
				qui replace `depvar' = rbinomial(`scan',`depvar') 
			}
			...
			mat li e(b)
			di
			di as txt "Position specified in pos() option: " as res `position'
			di
			...
			di as txt "(Note: Displayed coefficients are approximate, use only for checking input.)"
			di as txt "{hline 78}"
		restore
		exit
	}
	...
	null(`null')		///
	scan(`scan') 		///
	add(`addscalar') `expb' `inside' `nodots' `silent' `pll'
...
* 	-_psim0- version 1.0 JL 30January2013
...
/* If the parallel option is activated */
			if (length("`pll'")) {
				global cmd `cmd'
				global using `using'
				global add `add'
				...
				global exbp `expb'
				global null `null'
				global nodots `nodots'
				parallel, nodata prog: _pll_psim1 `s', b(`=`m'') nreps(`=`nreps'') alpha(`=`alpha'') cmd($cmd) using($using) pos(`=`pos'') fam($fam) null($null) scan($scan) add($add) $expb $inside $nodots

				/* Appending results */
				use `r(pll_id)'1
				forval i = 1/$PLL_CLUSTERS {
					append using `r(pll_id)'`i'
				}
			}
/* Otherwise (serial fashion) */
			else {
				_psim1 `s', b(`m') 		///
				nreps(`nreps') 			///
				alpha(`alpha')		 	///
		...
		loc ms = subinstr("`m'", "." ,"_",1)
		loc ms = subinstr("`ms'", "-" ,"neg",1)
	...
	foreach bs of local b {
		loc lab = "`bs'"
		loc bs = subinstr("`bs'", "." ,"_",1)
		loc bs = subinstr("`bs'", "-" ,"neg",1)
	...
	* means
	forval i=1/`ncov' {
		if inlist("`dist`i''", "normal", "poisson") == 1 {
			loc m`i' : word 4 of `cov`i''
		}
	}
	* sds
	forval i=1/`ncov' {
		if inlist("`dist`i''", "normal") == 1 {
			loc sd`i' : word 5 of `cov`i''
		}
	}		
	...
	if "`corr12'" != "" | "`corr13'" != "" | "`corr23'" != "" {
		...
		if `wc'==3 {
			loc matc = "matrix __C = (1, `c1', `c2', 1, `c3', 1)"
		}
		if `wc'==2 & `idx'==7 {
			loc matc = "matrix __C = (1, `c1', `c2', 1, 0, 1)"
		}
		if `wc'==2 & `idx'==8 {
			loc matc = "matrix __C = (1, `c1', 0, 1, `c3', 1)"
		}
		if `wc'==2 & `idx'==9 {
			loc matc = "matrix __C = (1, 0, `c2', 1, `c3', 1)"
		}
		if `wc'==1 & `idx'==3 {
			loc matc = "matrix __C = (1, `c1', 1)"
		}
		if `wc'==1 & `idx'==4 {
			loc matc = "matrix __C = (1, `c2', 1)"
		}
		if `wc'==1 & `idx'==5 {
			loc matc = "matrix __C = (1, `c3', 1)"
		}
...
function sims()
{
	...
	for (i=1; i<=M; i++) {
		...
		stata("qui count if !missing(y)")
		stata("cap assert r(N) == \`N'")
		stata(`"if _rc local fail \`iter'"')
		fail = strtoreal(st_local("fail"))
