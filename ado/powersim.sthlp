...
{synopt :{opth f:amily(powersim##familyname:familyname)}} error distribution{p_end}
{synopt :{opth l:ink(powersim##linkname:linkname)}} link function{p_end}
...
{synopt :{opt inter1(# cov#*cov#)}} Interaction effect specification {p_end}
{synopt :{opt inter2(# cov#*cov#)}} Interaction effect specification {p_end}
{synopt :{opt inter3(# cov#*cov#)}} Interaction effect specification {p_end}
...
{synopt :{opt igaussian} [{it:#}]}inverse Gaussian, with shape parameter {it:#}{p_end}
...
{synopt :{opt nbinomial} {it:#}}negative binomial, with overdispersion {it:#}{p_end}
...
and {help glm} commands can be used for specifying the analysis model. 
...
{phang}
{opth b(numlist)} List of effect sizes.
...
{phang}
{opt inter1(# cov#*cov#)} specifies an interaction effect; for example, to generate data 
with an interaction effect of 0.5 for cov2 and cov3, type {cmd:inter1(0.5 cov2*cov3)}.

{phang}
{opt inter2(# cov#*cov#)} specifies an additional interaction effect. 

{phang}
{opt inter3(# cov#*cov#)} specifies an additional interaction effect.
...
{phang}
{opt gen:data} creates a single realization of the data; no simulations are performed when this 
option is specified. 
...
{phang}
{opt add:scalar} Specifies to fetch a scalar that is available in {opt e()} of the analysis model from each MC replication; for example, if you would like to record the value of the log-likelihood from each replication of a {opt glm}, the option would be specified as addscalar(ll); the scalar will then be appended to the resulting dataset which contains results from each replication.
...
{opt glm}. Note that {opt powersim} removes the current data from memory and replaces 
it with simulation results; make sure to save your data before running powersim. Also note
that {cmd powersim} is creating a scalar with the name _bp. In case that a scalar with
...
{pstd}
Available distributional families are
...
{pstd}Estimating power for various effect and sample sizes for a linear predictor 
that is correlated with another predictor variable in a linear model: {p_end}
...
{pstd}_bp is a placeholder for the effect sizes specified in {opt b()}; x1 and x2 are drawn 
from a multivariate normal distribution with r = 0.5. {p_end}

{hline}

{pstd}Using an existing do-file:{p_end}
...
{pstd}Poisson model with correlated predictor variables and power estimated for the 
interaction effect of the two variables:{p_end}
...
{phang2}{cmd: corr12(0.5)					///} {p_end}
{phang2}{cmd: inside 						///} {p_end}
{phang2}{cmd: saving(psim_results, replace)	///} {p_end}
{phang2}{cmd: dofile(psim_dofile, replace) : glm y c.x1##c.x2, family(poisson) link(log) } {p_end}
...
{phang2}{cmd: cons(0.5)						///} {p_end}
{phang2}{cmd: dofile(psim_dofile, replace) : glm y i.x1##i.x2, family(binomial) link(logit)} {p_end}
...
and a single binomial predictor, distributed Bi(1,0.5), for a fixed sample
...
{phang2}{cmd: inside 					///} {p_end}
{phang2}{cmd: dofile(psim_dofile, replace) : glm y x1, family(nbinomial ml) link(log) } {p_end}
...
{phang2}{cmd: dofile(psim_dofile, replace) : glm y i.x1, fam(igaussian) link(power -2)} {p_end}
...
{synopt:{cmd:r(niter)}}number of Monte Carlo replications{p_end}
